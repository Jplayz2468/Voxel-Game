    ...
    setupNetworkHandlers() {
        ...
        this.networkClient.onMessage('meshUpdate', (data, timestamp) => {
            ...
            if (totalMessageTime > 16) { // More than one frame at 60fps
                console.warn(`⚠️ SLOW TERRAIN UPDATE: ${totalMessageTime.toFixed(2)}ms (>16ms frame budget) - Check performance debugger!`);
            }
        });
        ...
        this.networkClient.onMessage('chunkUpdate', (data, timestamp) => {
            ...
            if (totalMessageTime > 8) {
                console.warn(`⚠️ SLOW CHUNK UPDATE: ${totalMessageTime.toFixed(2)}ms (should be <8ms)`);
            }
        });
        ...
        this.networkClient.onMessage('voxelsUpdate', (data, timestamp) => {
            ...
            console.log(`⏱️ Voxel update processing: ${processingTime.toFixed(2)}ms`);
        });
